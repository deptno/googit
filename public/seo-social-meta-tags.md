...
description: OGtag, Twitter 태그 작업에 대한 작지만 치명적인 기록.
tags:
  - seo
  - ogtag
  - twittertag
...
트러블 슈팅에 좀 더 도움이 될만한 그리고 내가 겪은 이슈들을 정리해봤다. 매를 다 맞으면서 만들어서 더이상은 이슈 나올 것도 없어보인다.
...
특히 응답에서 헤더를 잘 못 내려주거나하게 되면 특정 서비스에서만 오류가 난다. 서비스업체가 그 헤더를 보는가 안 보는가는 구현의 이슈이기 때문에 구현체가 다르므로 어떤 곳에서만 동작을 안하고 태그는 잘 심어져 있다라고 한다면 **헤더**를 먼저 의심해보도록 하자.
